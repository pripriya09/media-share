...
function LogMedia() {
  ...
  const getUserDetails = async (userAccessToken) => {
    try {
    ...
    try {
      ...
      if (pages && pages.length > 0) {
        ...
        if (targetPage) {
          ...
          getInstagramAccountDetails(instagramBusinessAccountId, pageAccessToken);
        } else {
  ...
  const getInstagramAccountDetails = async (instagramBusinessAccountId, pageAccessToken) => {
    try {
      const response = await axios.get(
        `https://graph.facebook.com/v21.0/${instagramBusinessAccountId}`,
        {
          params: { access_token: pageAccessToken, fields: "id,username" },
        }
      );

      const { id, username } = response.data;
      console.log("Instagram Business Account ID:", id);
